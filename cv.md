# Entle Myezo

**Student** | Cape Peninsula University of Technology  
 071 234 5678 | 230076238@mycput.ac.za  
🔗 [GitHub](https://github.com/entle-myezo)

---

## Education

**Diploma in Information and Communication Technology** (2024 – present)  

***Matric***
- Completed 2022
---

## Skills

- **Programming Languages:** Java, Python, SQL, HTML/CSS  
- **Tools:** Git, GitHub, VS Code, MySQL Workbench  
- **Soft skills:** Problem-solving, team collaboration, technical communication

---
## Certifications & Training

**Work Readiness Programme (Call Centre Operations)** – JobsFund11  
*Certificate of Completion – February 2026*

**Technical competencies gained:**
- Customer relationship management (CRM) systems and ticketing workflows
- Troubleshooting common technical issues using decision trees and knowledge bases
- Professional communication and escalation protocols for IT support
- Data capture and record-keeping in service desk environments
- Time management and handling high-volume service requests

This programme strengthened my ability to communicate technical information clearly, manage user tickets, and follow structured problem-solving processes – key skills for helpdesk and technical support roles.

**Life Skills Course** – MRP Foundation Jumpstart  
*Certificate of Excellence*

**Skills developed:**
- Workplace communication and professional behaviour
- Time management and reliability
- Conflict resolution and teamwork
- Basic customer service principles
- Personal development and goal setting

**Relevance:**  
This certificate demonstrates my ability to work well in a team, follow workplace etiquette, and communicate clearly – essential soft skills for any professional environment, including IT.


---

## Projects (summary)

- [Project 1] – [Myezo Eatery Website]
- An interactive web application coded using Java, JavaScript, and CSS to allow users to browse products and prices and also placing online orders.


---

## References

Available upon request.
